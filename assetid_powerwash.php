            ...
            if ($counter == 1){
                if ($_SERVER["REQUEST_METHOD"] == "GET") {
                    if (isset($_GET['searchterm']) && !empty($_GET['searchterm'])){
                        ...
                        $command1 = sprintf("$GAMpath issuecommand cros query asset_id:%s command remote_powerwash doit", $mysearch);
                        exec($command1,$infoBasic);

                        echo "<h3>Performing a remote powerwash on device with Asset ID <font color='#008CBA'>$mysearch</font>. Here's what happened:</h3>";
