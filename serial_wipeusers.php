            ...
            if ($counter == 1){
                if ($_SERVER["REQUEST_METHOD"] == "GET") {
                    if (isset($_GET['searchterm']) && !empty($_GET['searchterm'])){
                        ...
                        $command1 = sprintf("$GAMpath issuecommand cros query id:%s command wipe_users doit", $mysearch);
                        exec($command1,$infoBasic);

                        echo "<h3>Clearing user profiles on device with Serial Number <font color='#008CBA'>$mysearch</font>. Here's what happened:</h3>";
